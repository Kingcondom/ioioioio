...
### 3.4 หัวข้อที่ออกสอบบ่อย — `data/<ระบบ>_hot.json` (array)

แสดงเป็นแท็บ **"ออกบ่อย"** ในเว็บ และเป็น **ส่วนที่ 0** ของ PDF กับไฟล์ export — หนึ่ง object ต่อหนึ่ง lecture

```json
{
  "lec": "13",
  "lecture": "Ischemic heart diseases (AL) (อ.สุรพันธ์)",
  "level": "ออกหนักที่สุดของ Cardio",
  "mcq":  ["ประเด็นที่ถูกถามซ้ำในรูปแบบ MCQ", "..."],
  "meq":  ["โจทย์ MEQ ที่เคยออก"],
  "osce": ["สถานี OSCE/SAQ ที่เคยออก"],
  "must": ["สิ่งที่ต้องตอบให้ได้ ไม่ตอบไม่ได้คะแนน"],
  "seen": "MED28–MED35 ทุกรุ่น ทั้ง MCQ, MEQ และ OSCE/SAQ",
  "ids":  ["C-MCQ-11", "C-OLD-33", "C-OSCE-02"]
}
```

- `ids` คือข้อตัวอย่างในคลังชุดเดียวกัน — บนเว็บจะเป็นปุ่มกดแล้ว **กระโดดไปทำข้อนั้นทันที** (สลับแท็บให้เอง)
- build จะ **หยุด** ถ้า `lec` ไม่มีในปฏิทิน หรือขาด `lecture` / `level` / `mcq` และจะ **เตือน** ถ้า `ids` อ้างข้อที่ไม่มีในคลัง
- ผูกเข้าระบบด้วย `"hot": ["data/<ระบบ>_hot.json"]` ใน `config.sets[]`

### 3.5 เพิ่มระบบใหม่ใน `config.json`
